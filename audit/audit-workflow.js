export const meta = {
  name: 'mk-site-adversarial-audit',
  description: 'Line-by-line adversarial bug hunt across the mk portfolio site, with runtime Playwright probes and multi-lens verification',
  phases: [
    { title: 'Find', detail: '16 finders: runtime probes + per-file static analysis + cross-contract + resilience' },
    { title: 'Verify', detail: 'two adversarial lenses per finding, refute-by-default' },
    { title: 'Critic', detail: 'completeness critic: what was missed' },
  ],
}

const SITE = '/tmp/claude-0/-home-user-mkss/62ba9b9c-0f4f-5dc3-b0ac-bed87a749296/scratchpad/site'
...
phase('Find')
log(`Auditing ~8,900 lines across 14 files with ${FINDERS.length} finders`)
...
const dead = FINDERS.length - alive.length
...
log(`${raw.length} raw findings from ${alive.length} finders`)

const seen = new Map()
for (const f of raw) {
  const k = `${(f.file || '').toLowerCase()}::${f.line}::${(f.category || '').toLowerCase()}`
  const prev = seen.get(k)
  if (!prev) { seen.set(k, { ...f, corroborators: [f.finder] }) }
  else { prev.corroborators.push(f.finder) }
}
const deduped = [...seen.values()]
log(`${deduped.length} unique findings after dedup (${raw.length - deduped.length} duplicates merged)`)

phase('Verify')

const LENSES = [
  { key: 'refute', ask: `Your job is to REFUTE this finding. Open the file, read the cited line and everything around it that matters, and look hard for the reason this is NOT a bug: a guard elsewhere that already handles it, a CSS rule that already covers it, a misread of the code, a wrong line number, a scenario that cannot actually occur, or a claim about an API that is wrong. Default to refuted=true when you are not certain the bug is real. Only set refuted=false if you can state the failing path line by line.` },
  { key: 'reproduce', ask: `Your job is to REPRODUCE this finding, empirically where possible. If it is a runtime or layout claim, drive it in Playwright at http://localhost:8099/index.html (export NODE_PATH=/opt/node22/lib/node_modules) and show the measurement. If it is a static-analysis claim about a code path, trace that path line by line and quote each step. If you cannot reproduce or trace it end to end, set refuted=true. Also judge whether the proposed fix would actually work and whether it would break anything else — if not, put the corrected fix in fixCorrection.` },
]
...
const verified = await parallel(deduped.map(f => () =>
  parallel(LENSES.map(l => () =>
    agent(
      ENV +
      `\n\nVERIFY THIS FINDING — lens: ${l.key}\n` +
      `file: ${f.file}\nline: ${f.line}\nseverity: ${f.severity}\ncategory: ${f.category}\n` +
      `title: ${f.title}\ndetail: ${f.detail}\nfailure: ${f.failure}\nproposed fix: ${f.fix}\nreported evidence: ${f.evidence}\n\n` +
      l.ask,
      { label: `verify:${l.key}:${f.file}:${f.line}`, phase: 'Verify', schema: VERDICT_SCHEMA }
    )
  )).then(votes => {
    const v = votes.filter(Boolean)
    if (!v.length) return null
    const kills = v.filter(x => x.refuted).length
    return { ...f, votes: v, survives: kills === 0, kills, voteCount: v.length }
  })
))

const scored = verified.filter(Boolean)
const confirmed = scored.filter(x => x.survives)
const contested = scored.filter(x => !x.survives && x.kills < x.voteCount)
const killed = scored.filter(x => x.kills === x.voteCount)
log(`${confirmed.length} confirmed by both lenses · ${contested.length} contested (1 of 2 refuted) · ${killed.length} refuted by both`)
...
const critique = await agent(
  ENV +
  `\n\nYou are the COMPLETENESS CRITIC for an audit that has just finished. ${FINDERS.length} finders swept the site and produced these ${confirmed.length} findings that survived two adversarial verifiers:\n\n${brief || '(none)'}\n\n` +
  `Contested findings (one verifier refuted them), for your awareness:\n${contested.map(f => `- ${f.file}:${f.line} — ${f.title}`).join('\n') || '(none)'}\n\n` +
  `Your job: find what this audit MISSED. Not a summary — new defects.\n` +
  `Ask yourself: which files or line ranges got the least attention? Which failure modes were never exercised (a second visit with a hash in the URL, resizing mid-scroll, rotating a phone, opening two overlays, filtering while the rail is pinned, a very long form input, back/forward navigation, printing the page)? Which of the site's own extensive markup comments make claims that nobody checked? Are there whole categories untouched — print styles, RTL (the code comments mention Arabic may be added later), i18n, security of the WhatsApp link construction, the noscript path, the favicon set?\n` +
  `Go and actually look, run what you can in Playwright, and report NEW findings in the same schema. Report nothing rather than padding.`,
  { label: 'critic:completeness', phase: 'Critic', schema: FINDINGS_SCHEMA }
)

const extra = (critique && critique.findings ? critique.findings : []).map(f => ({ ...f, finder: 'critic', corroborators: ['critic'] }))
log(`critic surfaced ${extra.length} additional candidate findings`)

const extraVerified = await parallel(extra.map(f => () =>
  agent(
    ENV +
    `\n\nVERIFY THIS FINDING — lens: refute\nfile: ${f.file}\nline: ${f.line}\nseverity: ${f.severity}\ncategory: ${f.category}\ntitle: ${f.title}\ndetail: ${f.detail}\nfailure: ${f.failure}\nproposed fix: ${f.fix}\nreported evidence: ${f.evidence}\n\n` +
    LENSES[0].ask,
    { label: `verify:critic:${f.file}:${f.line}`, phase: 'Critic', schema: VERDICT_SCHEMA }
  ).then(v => (v && !v.refuted) ? { ...f, votes: [v], survives: true, kills: 0, voteCount: 1 } : null)
))

const extraConfirmed = extraVerified.filter(Boolean)
log(`${extraConfirmed.length} of the critic's findings survived verification`)
