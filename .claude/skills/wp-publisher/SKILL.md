...
`WP_ALLOW_LOCAL_FETCH=1` (تنزيل صور من عناوين محلية)، `WP_SITE_GMT_OFFSET` (فرق توقيت
الموقع بالساعات)، `WP_MAX_RETRIES`، `WP_TIMEOUT`.
...
- **مقال في سلة المحذوفات بيترفض** — استعيده الأول أو غيّر الـ slug.
- **الجدولة بتوقيت الموقع** مش توقيت الجهاز. لو الأداة نبّهت إن الموقع رجّع حالة
  مختلفة عن المطلوبة، راجع التاريخ و`WP_SITE_GMT_OFFSET`.
- **`status: future` لازم معاه `date` مستقبلي** وإلا الأداة بترفض (ووردبريس كان بينشر فورًا).
...
```bash
cd wp-publisher && python3 -m unittest discover -s tests     # 190 اختبار، ~120 ثانية
```
