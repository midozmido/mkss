...
- **لو نبّه على تصادم slug** (مقال تاني برقم مختلف بنفس الـ slug) — بلّغ المستخدم واقترح
  تغيير الـ slug، متكملش وتكتب فوق مقال مش بتاعه.
...
```bash
cd wp-publisher && python3 -m unittest discover -s tests     # 129 اختبار، ~75 ثانية
```
