...
test('يجيب من بيانات العميل الحية لا من المقالات', () => {
  ...
  assert.match(last.body, /شغّال/, 'لم يذكر حالته الفعلية');
});
...
test('سؤال بلا إجابة ينتهي دائمًا بعرض الدعم البشري — لا طريق مسدود', () => {
  ...
  const last = chat.history(u).at(-1);
  assert.match(last.body, /كلم الدعم الفني/, 'لم يعرض التحويل لفريق الدعم');
...
test('النقلة تنقل السياق للأدمن — العميل لا يعيد كلامه', () => {
  ...
  assert.match(clientView.at(-1).body, /تم تحويلك لفريق الدعم/);
  assert.match(clientView.at(-1).body, /مش هتحتاج تعيد/, 'لم يُطمئن العميل أنه لن يعيد كلامه');
});
